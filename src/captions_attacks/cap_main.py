...
from src.shared_utils.shared_tools import make_allPairs, classifiers, city, DATAPATH
...
def attack_captions(th, sublinear):
    ...
    dataFile = "feature/" + str(sublinear) + str(th) + "filt_TFIDF.csv" #no save

    make_features(dataFile, df ,TFIDF_matrix,  allPairs, DATAPATH)

    score_all_aucs(dataFile, DATAPATH) #, classifiers=classifiers)
